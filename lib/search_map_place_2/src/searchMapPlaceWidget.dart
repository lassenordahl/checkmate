...
class _SearchMapPlaceWidgetState extends State<SearchMapPlaceWidget> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _containerHeight = Tween<double>(begin: 55, end: 299).animate(
      CurvedAnimation(
    ...
    return Center(
      child: Row(
        children: <Widget>[
          ...
          // GestureDetector(
          //   child: Icon(null, color: this.widget.iconColor),
  ...
  void _autocompletePlace(String input) async {
    ...
    if (input.length > 0) {
      ...
      if (json["error_message"] != null) {
        ...
        final predictions = json["predictions"].length > 3 ? json["predictions"].sublist(0, 4): json["predictions"];
