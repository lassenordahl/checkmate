...
class logisticClassify2(ml.classifier):
    ...
    def __init__(self, *args, **kwargs):
        ...
        """
        self.classes = [
            0, 1]              # (default to 0/1; replace during training)
...
## METHODS ################################################################
    ...
    def plotBoundary(self, X, Y):
        ...
        x2b = [(-1 * self.theta[0] - (self.theta[1] * x1val) / self.theta[2])
                for x1val in x1b]

        # Now plot the data and the resulting boundary:
        A = Y == self.classes[0];  # and plot it:
        plt.plot(X[A, 0], X[A, 1], 'b.', X[~A, 0], X[~A, 1],
                 'r.', x1b, x2b, 'k-'); plt.axis(ax); plt.draw();
...
#         print(self.theta)
        responses = [(sum([self.theta[i + 1] * val for i,
                      val in enumerate(x)]) + self.theta[0]) for x in X]
        Yhat = np.array([self.classes[1] if r > 0 else self.classes[0]
                        for r in responses])
...
#         def sigma(x, t):
#             return (1 + np.exp(-1 * (x * t)) ** -1)

#         def negative_log_likelihood(row_count):
#             return sum([(-YY[i] * np.log(sigma(XX[i] * self.theta.T)) - ((1 - YY[i]) * np.log(1 - (sigma(XX[i] * self.theta.T))))) for i in range(row_count)])
        ...
        while not done:
            stepsize, epoch = initStep*2.0 / \
                (2.0+epoch), epoch+1;  # update stepsize
            # Do an SGD pass through the entire data set:
            for i in np.random.permutation(M):
                ri = XX[i] * self.theta.T;
                gradi = sum([(-YY[i] + sigma(ri)) * XX[i][n]
                            for n in range(N)]);
            ...
            Jsur = negative_log_likelihood(M)
            Jnll.append(Jsur)  # TODO evaluate the current NLL loss
            plt.figure(1); plt.plot(Jnll, 'b-', J01,
                       'r-'); plt.draw();    # plot losses
            if N == 2: plt.figure(2); self.plotBoundary(
                X, Y); plt.draw();  # & predictor if 2D
            ...
            done = (epoch > stopEpochs) or (len(Jnll) > 2 and (abs(Jnll[-1] - Jnll[-2]) < stopTol);   # or if Jnll not changing between epochs ( < stopTol )
